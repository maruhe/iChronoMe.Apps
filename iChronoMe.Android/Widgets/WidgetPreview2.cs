...
{
    public class WidgetPreviewListAdapter2 : BaseAdapter
    {
        Activity mContext;
        Point wSize;
        ...
        public WidgetPreviewListAdapter2(Activity context, System.Drawing.Point size, DynamicCalendarModel calendarModel, EventCollection eventsMonth, EventCollection eventsList, Drawable wallpaperDrawable)
        {
            mContext = context;
            ...
            llDummy = new LinearLayout(context);
        ...
        Dictionary<int, ViewHolder2> ViewHolders = new Dictionary<int, ViewHolder2>();
        ...
        private class ViewHolder2 : Java.Lang.Object
        {
            ...
            if (RunningWidgetLoader < Math.Max(1, System.Environment.ProcessorCount - 1))
            {
                ...
                var loader = new Thread(() =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            int iPos = 0;
                            ViewHolder2 xHolder = null;
                            lock (ViewsToLoad)
                            {
                                xHolder = (ViewHolder2)ViewsToLoad[ViewsToLoad.Count - 1];
            ...
            try
            {
                ...
                else if (cfg is WidgetCfg_Clock)
                {
                    if (cfg is WidgetCfg_ClockAnalog)
                    {
                        ...
                        bmp = BitmapFactory.DecodeStream(wv.GetBitmap(DateTime.Now, iWidthPx, iHeightPx));
                    }
                }
                ...
                if (bmp == null)
                {
                    ...
                    {
                        swStart = DateTime.Now;
                        var vWidget = rv.Apply(mContext, llDummy);
                        rv.Dispose();
                        rv = null;
                        ...
                        if (cfg is WidgetCfg_CalendarTimetable)
                        {
                            if (myEventsList.Count > 0)
                            {
                                ...
                                */
                                ...
                                vWidget.FindViewById<ListView>(Resource.Id.event_list).Adapter = new EventListFakeAdapter2(mContext, cfg as WidgetCfg_CalendarTimetable, wSize, CalendarModel, myEventsList);
                            }
                        }
    ...
    public class EventListFakeAdapter2 : BaseAdapter
    {
        Context mContext;
        CalendarEventListRemoteViewsFactory viewsFactory;

        public EventListFakeAdapter2(Context context, WidgetCfg_CalendarTimetable cfg, Android.Graphics.Point size, DynamicCalendarModel calendarModel, EventCollection events)
        {
            mContext = context;
        ...
        public override Android.Views.View GetView(int position, Android.Views.View convertView, ViewGroup parent)
        {
            var v = viewsFactory.GetViewAt(position, false).Apply(mContext, parent);
            WidgetPreviewListAdapter2.RemoveClickListeners(v);
            return v;
        }
